...
public class OrdersCommDb {
    public static void ordersCommunicator(List<OrderModel> orderList) throws SQLException {
        try (Connection connection= ConnectionIntializerTask.getConnection())
        {
            transferAllOrdersPassedOnBatchesToMysql(connection,orderList);
        }catch (Exception e)
        {
            e.getStackTrace();
        }
...
//        if(!orderNumberExists) {
    ...
    public static void transferOrdersBatchToMysql(Connection connection, @NotNull List<OrderModel> batchlist) throws SQLException {
